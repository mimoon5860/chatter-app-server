...
class chatRouter extends abstractRouter {
    ...
    private callRouters() {
        ...
        // send msg 
        this.router.post('/send/msg', this.uploader.multiUploader('chat-images'), body('senderId').exists().customSanitizer(sanitizers.toObjectId), body('conversation').exists().customSanitizer(sanitizers.toObjectId), this.chatsController.sendMsgController);
